...
export function Contact() {
    ...
    const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
        // NO prevenimos el default todavía. Queremos que el form se envíe al iframe.
        ...
        // 1. Validación Manual
        if (!name?.trim() || !email?.match(/^[\w-.]+@([\w-]+\.)+[\w-]{2,}$/) || message?.trim().length < 10) {
            event.preventDefault(); // Solo cancelamos si hay error
            alert("Por favor completá todos los campos correctamente.");
            return;
        }

        // 2. Si pasa la validación, el navegador va a enviar el form al iframe.
        // Nosotros solo actualizamos la UI para que el usuario crea que es mágico.
        setStatus('submitting');

        // Simulamos el tiempo de red y mostramos éxito
        setTimeout(() => {
            setStatus('success');
            formRef.current?.reset(); // Limpiamos el form visualmente
            
            // Volvemos a estado normal después de 5 segundos
            ...
            <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-8 sm:mb-12">
                    <SectionTitle
                        ...
                        subtitle="Contame sobre tu propuesta y cómo puedo ayudarte a escalarla."
                    />
                </div>
                ...
                <div className="mt-8 grid gap-8 rounded-3xl border border-slate-800 bg-slate-900/40 p-6 sm:p-10 backdrop-blur-sm lg:grid-cols-2 lg:gap-12 shadow-xl">
                    ...
                    {/* Iframe invisible que recibe la respuesta del servidor */}
                    <iframe name="hidden_iframe" style={{ display: 'none' }}></iframe>

                    {/* Formulario HTML Real apuntando al iframe */}
                    <form 
                        ref={formRef}
                    ...
                    >
                        ...
                        <div className="grid gap-5 sm:grid-cols-2">
                            <label className="flex flex-col gap-2 text-sm font-medium text-slate-300">
                                ...
                                <input
                                    ...
                                    name="name"
                                    required
                                    className="rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3 text-slate-100 placeholder-slate-500 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-all"
                        ...
                        <label className="flex flex-col gap-2 text-sm font-medium text-slate-300">
                            ...
                            <textarea
                                ...
                                name="message"
                                required
                                className="rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3 text-slate-100 placeholder-slate-500 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-all resize-none"
                                placeholder="Cuéntame detalles sobre el proyecto..."
                            ></textarea>
                        </label>
                        ...
                        <div className="pt-2">
                            <Button 
                                type="submit" 
                                className="w-full sm:w-auto gap-2 cursor-pointer shadow-none active:scale-95 transition-transform" 
                        ...
                        {status === 'success' && (
                            <div className="flex items-start gap-3 text-sm text-emerald-300 bg-emerald-500/10 p-4 rounded-xl border border-emerald-500/20 animate-in fade-in slide-in-from-bottom-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-emerald-400/80">Gracias por escribirme. Te responderé a la brevedad.</p>
                                </div>
                            </div>
                        )}
                    </form>
